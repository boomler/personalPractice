...
function doValidate(formRef) {
	try {
		...
		if (typeof (formRef) == "string") {
			...
			formId=formId==null||formId==""?formRef.getAttribute("name"):formId;
		}
...
ValidationFramework.validateField = function(field) {
	...
	var retStr = "OK";
	//校验配置的display-name-key是否存在
  	if(field.getDisplayNameKey()!=null && !field.getDisplayNameKey()){
	  if(!nameResource[field.getDisplayNameKey()]){
	     return field.getDisplayNameKey()+":未配置！";
	  }
	}
	var retStr = "OK";
...
ValidationFramework.validateDepend = function(field, depend) {
	if (depend.getName() == "required") {
	...
	}else if (depend.getName() == "other") {
		var method=depend.getParams()[5];
		var is=false;
		try{
			is = eval(method+"()");
		}catch(e){
		...
		return is;
	} else if (depend.getName() == "length") {
		return ValidateMethodFactory.validateLength(field, depend.getParams());
	 }else {
...
ValidationFramework.getErrorString = function(field, depend) {
	...
	if (typeof retStr != 'string'&&typeof depend.getParams()[6]!="string") {
		retStr = stringResource["default"];
		if(field.getDisplayNameKey()!=null){
		  retStr = retStr.replace("{0}", nameResource[field.getDisplayNameKey()]);
		}else{
		  retStr = retStr.replace("{0}", field.getDisplayName());
		}
		return retStr;
	}else if(typeof depend.getParams()[6]=="string"){
		retStr = depend.getParams()[6];
	}
	//配置了displayNameKey时读取 nameResource["displayNameKey"]的内容
	if(field.getDisplayNameKey()!=null){
		  retStr = retStr.replace("{0}", nameResource[field.getDisplayNameKey()]);
	}else{
		  retStr = retStr.replace("{0}", field.getDisplayName());
	...
	if (dep == "minlength" || dep == "maxlength" || dep == "date" ) {
		...
		for(var i=1;i<5;i++){
			var reInt = retStr.indexOf("{"+i+"}");
			if(reInt>-1){
				retStr = retStr.replace("{"+i+"}", depend.getParams()[i]);
			}
		}
	}
...
function ValidationField() {
  ...
  this.getDisplayNameKey = function() { return this._displayNameKey; }
	this.setDisplayNameKey = function(p0) { this._displayNameKey = p0; }
...
FieldFactory.getFieldFromDOM = function(dom) {
	...
	field.setDisplayNameKey(dom.getAttribute("display-name-key"));	
	...
	if (dom.hasChildNodes()) {
		...
		for (var i = 0; i < depends.length; i++) {
			...
			dp.addParam(item.getAttribute("method"));
			dp.addParam(item.getAttribute("message"))
			field.addDepend(dp);
		}
	}
	return field;
...
ValidateMethodFactory.validateRequired = function(field, params) {
	var obj = ValidationFramework.getWebFormFieldObj(field);
	//去除前后空格后校验 Lei Liangfa 2008.11.19
	if (obj == null || obj.value.trim()=="") return false;
...
ValidateMethodFactory.validateMinLength = function(field, params) {
	...
	var temp =  obj.value.replace(/[^\x00-\xff]/g,"xx");
	temp = temp.replace(/·/g,"xx");
...
ValidateMethodFactory.validateLength = function(field, params) {
	var obj = ValidationFramework.getWebFormFieldObj(field);
	if (obj == null) return false;
	if(obj.value==""){return true;}
	if (!isdisabled&&obj.disabled == true) return true;
	var v = ValidationFramework.getIntegerValue(params[0]);
	var temp =  obj.value.replace(/[^\x00-\xff]/g,"xx");
	temp = temp.replace(/·/g,"xx");
	return (temp.length == v);
...
ValidationErrorString["zh-cn"]["commonchar"] = "{0}必须是普通英文格式字符：字母，数字和下划线。";
...
ValidationErrorString["zh-cn"]["other"]="other";
ValidationErrorString["zh-cn"]["length"] = "{0}长度必须为{1}位";
...
ValidationErrorString["en-us"]["other"]="other";
